...
if (!empty($list))
{
	...
	foreach($list as $num_row => $row){ 
		if ($row->{"is_read"}===0 || $row->{"is_read"}==null)
			$temp[] = $row->{"notification_id"};
	}
	$tempstr = implode('&ids[]=', $temp);
	$url = site_url("notification/ajax_inbox_mark_read")."?ids[]=".$tempstr;
	if (!empty($temp))
	{
		echo "<tr><script src='/".$this->default_javascript_path.'/'.grocery_CRUD::JQUERY."'></script><script>var count = ".count($temp)." ; $.get( '".$url."' ); </script></tr>";
	}
}
?>  
...
<?php foreach($list as $num_row => $row){ ?>
    <tr id="row-<?php echo $row->{"notification_id"} ?>" class='<?php if ($row->{"is_read"}===0 || $row->{"is_read"}==null) echo "unread";?>'>
