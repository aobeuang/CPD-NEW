...
jQuery(function ($) {
    ...
    $('#field-WORKFLOW').change(function(){
    	console.log($(this).val());
    	
    	$.ajax({
	          url: "/index.php/document/getValue_Task/"+$(this).val(),
	          type: "get",
	          success: function(data){
	        	  var obj = JSON.parse(data);
//	        	  console.log(obj.BARCODEDOCTYPE);
	        	  for(i=0;i<obj.length;i++){
	        		  $('.'+obj[i].key+'_form_group').find('div').html(''+obj[i].value+'');
	        	  }
	        	 /* if(typeof obj.DOC_TYPE !='undefined'){
		        	  $('.DOC_TYPE_form_group').find('div').html(''+obj.DOC_TYPE+'');
		        	  $('.ASSIGNEE_form_group').find('div').html(''+obj.ASSIGNEE+'');
		        	  $('.BARCODEDOCTYPE_form_group').find('div').html(''+obj.BARCODEDOCTYPE.toUpperCase()+'');
	        	  }else{
	        		  $('.DOC_TYPE_form_group').find('div').html('ไม่มีชนิดเอกสาร');
		        	  $('.ASSIGNEE_form_group').find('div').html('ไม่มีผู้รับงาน');
		        	  $('.BARCODEDOCTYPE_form_group').find('div').html('ไม่มีบาร์โค้ดเอกสาร');
	        	  }*/
	          	}
			});
    });
