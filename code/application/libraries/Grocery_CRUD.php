	...
	protected function change_list_value($field_info, $value = null)
	{
		...
		switch ($real_type) {
			...
			case 'datetime':
				...
				$temp = strtotime($value);
				$value = date('YY-mm-dd h:i:s', $temp);
...
class grocery_CRUD_Model_Driver extends grocery_CRUD_Field_Types
{
	...
	protected function db_insert($state_info)
	{
		...
		if($validation_result->success)
		{
			...
			if($this->callback_insert === null)
			{
				...
				// strip all tags and special char
				// ntksoftware
				foreach ($insert_data as $k=>$val)
				{
					$insert_data[$k] = strip_tags($val);
				}
	...
	protected function get_list()
	{
		...
		if(!empty($this->or_like))
			foreach($this->or_like as $or_like)
			{
				/*if ("customer_id"==$or_like[0])
				{
					print_r($this);
					die();
					
					$or_like[0] = "customers.".$or_like[0];
				}*/
				
				$should_do = true;
				if (!empty($this->state_info->search))
				{
					foreach ($this->state_info->search as $k=>$v)
					{
						if ($or_like[2]=="both" && $v==$or_like[1] && $or_like[0]=="customer_id")
						{
							$should_do = false;
						}
					}
		...
		if(!empty($this->relation_reverse))
			foreach($this->relation_reverse as $relation)
				$this->basic_model->join_relation_reverse($relation[0],$relation[1],$relation[2],$relation[3]);
					
		if(!empty($this->relation_reverse_sum)){
			foreach($this->relation_reverse_sum as $relation)
				$this->basic_model->join_relation_sum($relation[0],$relation[1],$relation[2],$relation[3]);
		}
		...
		if(!empty($this->relation_reverse)){
			...
			$results = $this->basic_model->get_list_reverse();
		}
		
		if(!empty($this->relation_reverse_sum)){
			$results = $this->basic_model->get_list_reverse_sum();
		}else
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function load_js_chosen()
	{
		...
		$this->set_js_lib($this->default_javascript_path.'/jquery_plugins/jquery.chosen.new.js');
	}
	...
	protected function get_dropdown_input($field_info,$value)
	{
		...
		$input = "<select id='field-{$field_info->name}' name='{$field_info->name}' class='chosen-select-new' data-placeholder='".$select_title."'>";
...
class grocery_CRUD_States extends grocery_CRUD_Layout
{
	...
	protected $return_related_table = array();
	
	
	public function getReturn_related_table(){
		return $return_related_table;
	}
	public function setReturn_related_table($return_related_table){
		$this->return_related_table = $return_related_table;
	}
...
class Grocery_CRUD extends grocery_CRUD_States
{
	...
	/*
	 * customize set character limiter
	 */
	public function set_character_limiter($limit=30){
		$this->character_limiter = $limit;
		return $this;
	}
	 ...
	 * Enter description here ...
	 */
	protected function get_columns_ntk()
	{
		if($this->columns_checked === false)
		{
			$field_types = $this->get_field_types();
			...
			foreach($this->columns as $col_num => $column)
			{
				...
				if(isset($this->relation_ntk[$column]))
				{
	
					$new_column = $this->_unique_field_name($this->relation_ntk[$column][0]);
					$this->columns[$col_num] = $new_column;
				...
				{
					if(!empty($this->relation_ntk))
					{
						$table_name  = $this->get_table();
						foreach($this->relation_ntk as $relation)
						{
							if( $relation[2] == $column )
							{
								$new_column = $table_name.'.'.$column;
...
// 		$this->character_limiter = $this->config->character_limiter;
	 ...
	 * @return Grocery_CRUD
	 */
	public function set_relation_reverse($field_name , $related_table, $related_title_field, $where_clause = null, $order_by = null)
	{
		$this->relation_reverse[$field_name] = array($field_name, $related_table,$related_title_field, $where_clause, $order_by);
		return $this;
	}
	 ...
	 * @return Grocery_CRUD
	 */
	public function set_relation_reverse_sum($field_name , $related_table, $related_title_field, $where_clause = null, $order_by = null)
	{
		$this->relation_reverse_sum[$field_name] = array($field_name, $related_table,$related_title_field, $where_clause, $order_by);
		return $this;
	}
	//End customize
