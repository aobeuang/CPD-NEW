...
class Auth_model extends MY_Model {
	...
	public function login_update( $user_id, $login_time, $session_id )
	{
		...
		$this->db->set( $data );
		$this->db->set('"last_login"',"TO_DATE('$time','yyyy-mm-dd hh24:mi:ss')",FALSE);
		...
		$this->db->set( $data );
		$this->db->set('"login_time"',"TO_DATE('$time','yyyy-mm-dd hh24:mi:ss')",FALSE);
		$this->db->insert( config_item('auth_sessions_table'));
	}
	...
	public function check_login_status( $user_id, $login_time )
	{
		...
		$select_columns_str = implode(",", $selected_columns);
		$user_table = "\"".config_item('user_table')."\""." u ";
		$session_table = "\"".config_item('auth_sessions_table')."\"". " s ";
		$sql = "SELECT $select_columns_str 
		FROM $user_table 
		LEFT JOIN $session_table on u.\"user_id\" =s.\"user_id\"
		WHERE
			s.\"user_id\" = $user_id
			AND s.\"login_time\" = to_date('$login_time','yyyy-mm-dd hh24:mi:ss')
		";
		...
		$this->db->select( $selected_columns )
			...
			->where( 's.login_time', "to_date('$login_time','yyyy-mm-dd hh24:mi:ss')");
		...
		else
		{
			...
			$session_id = $this->session->pre_regenerated_session_id;
	...
	public function clear_expired_holds()
	{
		...
		$temp = "TO_DATE('$expiration','yyyy-mm-dd hh24:mi:ss')";
		
		$sql = "DELETE FROM \"".config_item('IP_hold_table')."\"  WHERE \"time\"< $temp";
		$query = $this->db->query($sql);

		$sql = "DELETE FROM \"".config_item('username_or_email_hold_table')."\"  WHERE \"time\"< $temp";
		$query = $this->db->query($sql);		
	}
	...
	public function clear_login_errors()
	{
		...
		//$this->db->set('"time"',"TO_DATE('$time','yyyy-mm-dd hh24:mi:ss')",FALSE);
		
		$temp = "TO_DATE('$expiration','yyyy-mm-dd hh24:mi:ss')";
		
		$sql = "DELETE FROM \"".config_item('errors_table')."\"  WHERE \"time\"< $temp";
		$query = $this->db->query($sql);
		
		//$this->db->delete( config_item('errors_table'), ['time <' => $temp] );
	}
	...
	public function create_login_error( $data )
	{
		...
		$sequence_tablename = strtoupper("login_errors")."_SEQUENCE";
		...
		$this->db->insert( config_item('errors_table') );
	}
	...
	public function check_login_attempts( $string )
	{
		...
		if( $count1 == config_item('max_allowed_attempts') )
		{
			...
			$this->db->insert( config_item('IP_hold_table') );
		}
		...
		if( $string != '' )
		{
			...
			if( $count2 == config_item('max_allowed_attempts') )
			{
				...
				$this->db->set('ai',time());
				$this->db->set('"time"',"TO_DATE('$time','yyyy-mm-dd hh24:mi:ss')",FALSE);
				$this->db->insert( config_item('username_or_email_hold_table') );
			}
		}
	...
	protected function _insert_denial( $data )
	{
		...
		$this->db->insert( config_item('denied_access_table') );
	...
	public function auth_sessions_gc()
	{
		...
		{
			// clean old ci_session
			
			$fivedaysago = time()-5*24*60*60;
			
			$this->db->query('
				DELETE 
				FROM "' . $this->db->dbprefix(config_item('sessions_table')) .'"
				WHERE "timestamp" < '.$fivedaysago.'
			');
				
			// Immediately delete orphaned auth sessions
			$authsession_tablename = "\"".$this->db->dbprefix(config_item('auth_sessions_table'))."\"";
			$session_tablename = "\"".$this->db->dbprefix(config_item('sessions_table'))."\"";
			
			$this->db->query('
				DELETE FROM '.$authsession_tablename.' a
				WHERE a."id" not in 
					(SELECT c."id" FROM '.$authsession_tablename.' c
					LEFT JOIN '.$session_tablename.' b on b."id"=c."id"
					)
			');
