		...
		<div id="main-container" class="container-fluid col-md-12 col-xs-12 display">
			...
			<div id="right-container" class="col-md-12 col-xs-12">
				...
				<div class="row" id="canvas"> 	
						<center><h2  class="report-title-center" style="text-align:center">รายงานจำนวนสมาชิกสหกรณ์ ที่เป็นสมาชิก มากกว่า 1 แห่ง</h2></center>
						<center><h4>จำนวนสมาชิกของสหกรณ์   <span id="total"></span> คน</center></h4>	

				 		<div id="piechart" style="margin: auto;width: 900px;height: 350px;"></div>
...
<!-- 				<div> 
					...
					select  count(OU_D_ID) as num from moiuser.master_data where OU_D_FLAG in(1,2) and  NUMBER_OF_COOP =1 and DECODE(replace(translate(IN_D_COOP,'1234567890','##########'),'#'),NULL,'NUMBER','NON NUMER') = 'NUMBER'  and LENGTH (moiuser.master_data .IN_D_COOP) = 13 
					select  count(OU_D_ID) as num from moiuser.master_data where OU_D_FLAG in(1,2) and  NUMBER_OF_COOP =2 and DECODE(replace(translate(IN_D_COOP,'1234567890','##########'),'#'),NULL,'NUMBER','NON NUMER') = 'NUMBER'  and LENGTH (moiuser.master_data .IN_D_COOP) = 13 
					select  count(OU_D_ID) as num from moiuser.master_data where OU_D_FLAG in(1,2) and  NUMBER_OF_COOP =3 and DECODE(replace(translate(IN_D_COOP,'1234567890','##########'),'#'),NULL,'NUMBER','NON NUMER') = 'NUMBER'  and LENGTH (moiuser.master_data .IN_D_COOP) = 13 
					</textarea>
...
// 		console.log(listresult.result[1]);
        ...
        var options = {
          title: 'แสดงผลจำนวนสมาชิกที่อยู่ในสหกรณ์',
          width: 925,
     	 height: 350,
        ...
        google.visualization.events.addListener(chart, 'ready', function () {
      	...
      	btnSave.addEventListener('click', function () {
      		...
      		var docDefinition = { 
      				content: [
      					{
      						text: 'รายงานจำนวนสมาชิกสหกรณ์ ที่เป็นสมาชิก มากกว่า 1 แห่ง',
      						alignment: 'center',
      						style: 'header'
      					},
      					{
      		...
      		pdfMake.createPdf(docDefinition).download('รายงานจำนวนสมาชิกสหกรณ์ ที่เป็นสมาชิก มากกว่า 1 แห่ง.pdf');
      	  }, false);
