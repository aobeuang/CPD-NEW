...
select  sum(TOT_ANN),TYPE_NAME from TA_MEMBER_AN where TYPE_ANN = 'ประมง' GROUP BY TYPE_NAME

select  count(TOT_ANN) from TA_MEMBER_AN where TYPE_ANN = 'ประมง' 
					</textarea>
...
var datethai = "ที่มาข้อมูล สศก. ณ วันที่ "+ now.getDate()+ " " + thmonth[now.getMonth()]+ " พ.ศ. " + (0+now.getFullYear()+543);
  ...
  google.visualization.events.addListener(chart, 'ready', function () {
	...
	btnSave.addEventListener('click', function () {
		...
		var docDefinition = { 
				content: [
					{
						text: 'รายงานข้อมูลการทำประมง',
						alignment: 'center',
						style: 'header'
					},
					{
						text: 'จำนวนข้อมูลสัตว์เลี้ยงทั้งหมด '+parseFloat(listresult.total_animal).toLocaleString('en')+' ตัว',
						alignment: 'center',
						style: 'header',
						margin : [0,0,0,30],
					},
		...
		pdfMake.createPdf(docDefinition).download('รายงานข้อมูลการทำประมง.pdf');
	  }, false);
