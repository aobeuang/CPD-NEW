...
<div id="report" class="mis">
	...
	<div id="main-wrapper">
    <span>Page Code : MIS700</span>
		<div id="main-container" class="container-fluid col-md-12 col-xs-12 display">
			
			<h2><span class="glyphicon glyphicon-stats"></span> รายงานสถานภาพสมาชิกสหกรณ์ (โดยนับสมาชิกสหกรณ์ที่สังกัดสหกรณ์มากกว่า 1 แห่ง)</h2>
			...
			<div id="right-container" class="col-md-12 col-xs-12">
				...
				<div class="row" id="canvas"> 	 
						<center><h2  class="report-title-center" style="text-align:center">รายงานสถานภาพสมาชิกสหกรณ์ (โดยนับสมาชิกสหกรณ์ที่สังกัดสหกรณ์มากกว่า 1 แห่ง)</h2></center>
...
<!-- 				<div> 
					...
					select count(OU_D_ID) as num from moiuser.master_data where OU_D_FLAG in(1,2)  and DECODE(replace(translate(IN_D_COOP,'1234567890','##########'),'#'),NULL,'NUMBER','NON NUMER') = 'NUMBER'  and LENGTH (moiuser.master_data .IN_D_COOP) = 13 and OU_D_STATUS_TYPE   in (1,11,13)";
				
				select count(OU_D_ID) as num  from moiuser.master_data where  OU_D_FLAG in(1,2)  and DECODE(replace(translate(IN_D_COOP,'1234567890','##########'),'#'),NULL,'NUMBER','NON NUMER') = 'NUMBER'  and LENGTH (moiuser.master_data .IN_D_COOP) = 13 and OU_D_STATUS_TYPE  not in (1,11,13)";
			
			select count(OU_D_ID) as num  from moiuser.master_data where OU_D_FLAG in(1,2) and DECODE(replace(translate(IN_D_COOP,'1234567890','##########'),'#'),NULL,'NUMBER','NON NUMER') = 'NUMBER'  and LENGTH (moiuser.master_data .IN_D_COOP) = 13";
        ...
        google.visualization.events.addListener(chart, 'ready', function () {
      	...
      	btnSave.addEventListener('click', function () {
      		...
      		var docDefinition = { 
      				content: [
      					{
      						text: 'รายงานสถานภาพสมาชิกสหกรณ์ (โดยนับสมาชิกสหกรณ์ที่สังกัดสหกรณ์มากกว่า 1 แห่ง)',
      						alignment: 'center',
      						style: 'header'
      					},
      					{
      		...
      		pdfMake.createPdf(docDefinition).download('รายงานสถานภาพสมาชิกสหกรณ์ (โดยนับสมาชิกสหกรณ์ที่สังกัดสหกรณ์มากกว่า 1 แห่ง).pdf');
      	  }, false);
