...
<!--                </select>-->
                ...
                <select style="text-align: center!important;" class="col-md-7 form-control" name="family_status" id="status">
                <?php
//                $all_prov = count(count($data_getAllMaritalStatus));
                $family_status = $user_survey_data[strtoupper('family_status')];
                $status_selected = false;
                for($i=0;$i<=count($data_getAllMaritalStatus);$i++){
                    if(!empty($data_getAllMaritalStatus[$i]->STATUS_NAME)) {
                        if(trim($data_getAllMaritalStatus[$i]->STATUS_NAME != '')){

                            $selected = "";
                            if (trim($family_status)==trim($data_getAllMaritalStatus[$i]->STATUS_NAME))
                            {
                                $selected = " selected='selected' ";
                                $status_selected = true;
                            }

                            echo ' <option '.$selected.' value="' . $data_getAllMaritalStatus[$i]->STATUS_CODE . '">' . trim($data_getAllMaritalStatus[$i]->STATUS_NAME) . '</option>';
                        }
                    }
                }?>
                </select>
