...
<div id="report" class="nonmis">
	...
	<div id="main-wrapper">
		<span>Page Code : AN107</span>
		<div id="main-container" class="container-fluid col-md-12 col-xs-12">
			<h2><span class="glyphicon glyphicon-signal"></span> รายงานข้อมูลการเลี้ยงโคนม</h2>
			...
			<ul class="list-group">
				...
				<li class="list-group-item active"><a href="<?php echo site_url('report1/index7')?>">รายงานข้อมูลการเลี้ยงโคนม</a></li>
						...
						<div class="container-fluid col-md-3 col-xs-12" style="margin-top: 6px;">
							...
							<?php  
								...
								if (!empty($all_survey_years))
								{
									...
									{
										$selected = "";
										if (!empty($filter_year) && $k==$filter_year)
...
<!-- 				    </div>		 -->
				...
				<div class="row">
					...
					<input id="save-chart-pdf" type="button" value="Save as PDF" disabled style="display: none;" />
...
filter_year = Number(filter_year)+543;
  ...
  btnSave.addEventListener('click', function () {
		...
		var docDefinition = { 
				content: [
					{
						text: 'รายงานข้อมูลการเลี้ยงโคนม',
						alignment: 'center',
						style: 'header'
					},
					{
		...
		pdfMake.createPdf(docDefinition).download('รายงานข้อมูลการเลี้ยงโคนม.pdf');
	  }, false);
...
{
	$.ajax({
		url:'<?php echo site_url('ReportAnalytic/count_cow') ?>',
				dataType:'json',
				type:'GET',		
...
var id;
	...
	{
		var filter_year = $('#filter_year').val(); 
		var filter_province_hidden =	$('#filter_province_hidden').val();
		var filter_range_income_hidden = $('#filter_range_income').val();
