...
<div id="report" class="nonmis">
	...
	<div id="main-wrapper">
		<span>Page Code : AN104</span>
		<div id="main-container" class="container-fluid col-md-12 col-xs-12">
			<h2><span class="glyphicon glyphicon-signal"></span> รายงานปัญหาที่พบในการประกอบอาชีพ</h2>
			...
			<ul class="list-group">
				...
				<li class="list-group-item active"><a href="<?php echo site_url('report1/index4')?>">รายงานปัญหาที่พบในการประกอบอาชีพ</a></li>
				...
				<div class="row" id="action-bar">
					<div class="report-action-bar">
						...
						<div class="container-fluid col-md-8 col-xs-12"  style="margin-top: 6px;">				
							...
							<?php $listfilter_status= array("ปัญหาที่ดิน","ปัญหาแหล่งน้ำ","ปัญหาเมล็ดพันธุ์","ปัญหาปุ๋ยสารเคมี",
									"ปัญหาความรู้เทคโนโลยี","ปัญหาแนวโน้นความต้องการของตลาด","ปัญหาตลาดรับซื้อ","ปัญหาสถานที่เก็บผลผลิต","ปัญหาข้อมูลด้านการตลาด","ปัญหาความต้องการข้อมูลทางการตลาด","ปัญหาราคาที่ได้","ช่องทางข้อมูลด้านการตลาด");?>							
  ...
  google.visualization.events.addListener(chart, 'ready', function () {
	...
	btnSave.addEventListener('click', function () {
		...
		var docDefinition = { 
				content: [
					{
						text: 'รายงานปัญหาที่พบในการประกอบอาชีพ',
						alignment: 'center',
						style: 'header'
					},
					{
		...
		pdfMake.createPdf(docDefinition).download('รายงานปัญหาที่พบในการประกอบอาชีพ.pdf');
	  }, false);
...
var filter_status	= $('#filter_status').val();
	...
	if(filter_status == 1){
		filter = 'water_problem';
	}
	if(filter_status == 2){
		filter = 'seed_problem';		
	}
	if(filter_status == 3){
		filter = 'fertilizer_problem';		
	}
	if(filter_status == 4){
		filter = 'technology_problem';		
	}
	if(filter_status == 5){
		filter = 'trends_problem';		
	}
	if(filter_status == 6){
		filter = 'maket_problem';		
	}
	if(filter_status == 7){
		filter = 'storehouse_problem';		
	}
	if(filter_status == 8){
		filter = 'datamaket_problem';		
	}
	if(filter_status == 9){
		filter = 'wantmaket_problem';		
	}
	if(filter_status == 10){
		filter = 'price_problem';		
	}
	if(filter_status == 11){
		filter = 'data_receive';		
	}
