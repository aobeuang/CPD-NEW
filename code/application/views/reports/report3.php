...
<div id="report" class="nonmis">
	...
	<div id="main-wrapper">
		<span>Page Code : AN103</span>
		<div id="main-container" class="container-fluid col-md-12 col-xs-12">
			<h2><span class="glyphicon glyphicon-signal"></span> รายงานข้อมูลการผลิตทางการเกษตรในรอบปีที่ผ่านมา</h2>
			...
			<ul class="list-group">
				...
				<li class="list-group-item active"><a href="<?php echo site_url('report1/index3')?>">รายงานข้อมูลการผลิตทางการเกษตรในรอบปีที่ผ่านมา</a></li>
				...
				<div class="row" id="action-bar">
					<div class="report-action-bar">
						...
						<div class="container-fluid col-md-8 col-xs-12"  style="margin-top: 6px;">				
							...
							<?php $listfilter_status= array("ชนิดพันธุ์พืช","วิธีการขายผลผลิต","ช่วงรายได้และค่าใช้จ่ายทางการเกษตร","ความคิดเห็นในการขายผลผลิต","ปุ๋ย/ปราบศัตรูพืช","ชนิดสัตว์บก","ชนิดสัตว์น้ำ","ช่วงรายได้และค่าใช้จ่ายทางการเลี้ยงสัตว์");?>							
...
function drawType1(filter_year,filter_province_hidden,filter_range_income_hidden,filter_status) {
	...
	for(var i=0;i<list_type.list_data.length;i++)
	{
		
		var temp = [];
		temp.push(list_type.list_data[i].name+'',list_type.list_data[i].value);
		data_coop.push(temp);
	}
  ...
  google.visualization.events.addListener(chart, 'ready', function () {
	...
	btnSave.addEventListener('click', function () {
		...
		var docDefinition = { 
				content: [
					{
						text: 'รายงานข้อมูลการผลิตทางการเกษตรในรอบปีที่ผ่านมา',
						alignment: 'center',
						style: 'header'
					},
{
					...
					{
						text: 'แบ่งข้อมูลตาม'+filter_status+' พบข้อมูล ' +list_type.total.toLocaleString()+ ' แถว จากข้อมูลทั้งหมด ' +list_type.countall.toLocaleString()+ ' แถว',						
						alignment: 'center',
						style: 'header',
						margin : [0,0,0,30],
					},
		...
		pdfMake.createPdf(docDefinition).download('รายงานข้อมูลการผลิตทางการเกษตรในรอบปีที่ผ่านมา.pdf');
	  }, false);
...
function drawType2(filter_year,filter_province_hidden,filter_range_income_hidden,filter_status) {
	...
	var data_coop = [['ความคิดเห็น', 'ขายผลผลิตให้กับสหกรณ์','ขายผลผลิตให้กับพ่อค้า']];
	
	filter_year = Number(filter_year)+543;
	
	for(var i=0;i<list_type.list_data['ขายผลผลิตให้กับสหกรณ์'].length;i++)
	{
		var temp = [];
		temp.push(list_type.list_comment[i],list_type.list_data['ขายผลผลิตให้กับสหกรณ์'][i].value,list_type.list_data['ขายผลผลิตให้กับพ่อค้า'][i].value);
		data_coop.push(temp);
	}
	var data = google.visualization.arrayToDataTable(data_coop);
	...
	var options = {
	        title: 'ความคิดเห็นในการขายผลผลิต',
	        width: 800,
	       	 height: 350,
       		tooltip: { trigger: 'selection' }	       	
	      };
  ...
  google.visualization.events.addListener(chart, 'ready', function () {
	...
	btnSave.addEventListener('click', function () {
		...
		var docDefinition = { 
				content: [
					{
						text: 'รายงานความคิดเห็นในการขายผลผลิต',
						alignment: 'center',
						style: 'header'
					},
					{
...
function drawType3(filter_year,filter_province_hidden,filter_range_income_hidden,filter_status) {

	var data_coop = [['รายการ', 'ซื้อจากสหกรณ์','ซื้อจากพ่อค้า']];
	filter_year = Number(filter_year)+543;
	for(var i=0;i<list_type.list_data['ซื้อจากสหกรณ์'].length;i++)
	{
		var temp = [];
		temp.push(list_type.list_comment[i],list_type.list_data['ซื้อจากสหกรณ์'][i].value,list_type.list_data['ซื้อจากพ่อค้า'][i].value);
		data_coop.push(temp);
	}
	var data = google.visualization.arrayToDataTable(data_coop);
	var view = new google.visualization.DataView(data);
	...
	var options = {
	        title: 'ข้อมูลการใช้ปุ๋ย/ยาปราบศัตรูพืช',
	        width: 1000,
	       	 height: 450,
       		tooltip: { trigger: 'selection' }	       	
	      };
  ...
  google.visualization.events.addListener(chart, 'ready', function () {
	...
	btnSave.addEventListener('click', function () {
		...
		var docDefinition = { 
				content: [
					{
						text: 'ข้อมูลการใช้ปุ๋ย/ยาปราบศัตรูพืช',
						alignment: 'center',
						style: 'header'
					},
					{
...
function drawType4(filter_year,filter_province_hidden,filter_range_income_hidden,filter_status) {
	...
	var data_coop = [['รายการ', 'รายรับ','รายจ่าย']];
	filter_year = Number(filter_year)+543;
	for(var i=0;i<list_type.list_data['รายรับ'].length;i++)
	{
		var temp = [];
		temp.push(list_type.list_comment[i],list_type.list_data['รายรับ'][i].value,list_type.list_data['รายจ่าย'][i].value);
		data_coop.push(temp);
	}
	var data = google.visualization.arrayToDataTable(data_coop);
	var view = new google.visualization.DataView(data);
	...
	var options = {
	        title: 'จำนวนสมาชิกแบ่งตามช่วงรายรับ/รายจ่าย',
	        width: 800,
	       	 height: 350,
       		tooltip: { trigger: 'selection' }	       	
	      };
  ...
  google.visualization.events.addListener(chart, 'ready', function () {
	...
	btnSave.addEventListener('click', function () {
		...
		var docDefinition = { 
				content: [
					{
						text: 'จำนวนสมาชิกแบ่งตามช่วงรายรับ/รายจ่าย',
						alignment: 'center',
						style: 'header'
					},
					{
...
var filter_status	= $('#filter_status').val();
	...
	if(filter_status == 0){
		filter = 'count_type_plant';
	}
	if(filter_status == 1){
		filter = 'how2sell';
	}
	if(filter_status == 2){
		filter = 'count_income_expense_plant';		
	}
	if(filter_status == 3){
		filter = 'product_sale_comment';		
	}
	if(filter_status == 4){
		filter = 'count_fertilizer_report3';		
	}
	if(filter_status == 5){
		filter = 'count_animal_land';		
	}
	if(filter_status == 6){
		filter = 'count_animal_water';		
	}
	if(filter_status == 7){
		filter = 'count_income_expense_animal';		
	}
	...
	$.ajax({
				...
				success:function (result) {	
					...
					if(filter_status == 3){
						...
						old_element.parentNode.replaceChild(new_element, old_element);
						if(list_type.total !== 0){					
							google.charts.setOnLoadCallback(function(){drawType3(filter_year,filter_province_name,filter_range_income_name,filter_status_name)});				
						}else{
							$('#dual_x_div').empty();
							$('#save-pdf').prop('disabled', true);
						}
					}
					else if(filter_status == 7 || filter_status == 2){
						var old_element = document.getElementById("save-pdf");
						var new_element = old_element.cloneNode(true);
						old_element.parentNode.replaceChild(new_element, old_element);
						if(list_type.total !== 0){	
