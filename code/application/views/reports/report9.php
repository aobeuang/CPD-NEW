...
<div id="report" class="nonmis">
	...
	<div id="main-wrapper">
		<span>Page Code : AN109</span>
		<div id="main-container" class="container-fluid col-md-12 col-xs-12">
			<h2><span class="glyphicon glyphicon-signal"></span> รายงานข้อมูลพันธ์ข้าว</h2>
			...
			<ul class="list-group">
				...
				<li class="list-group-item active"><a href="<?php echo site_url('report1/index9')?>">รายงานข้อมูลพันธ์ข้าว</a></li>
...
<!--						<div class="container-fluid col-md-6 col-xs-12"  style="margin-top: 6px;">		-->		
<!-- 							<label for="filter_plant">พันธ์ข้าว: </label>		 -->
<!-- 							<input type="hidden" id="filter_plant_hidden" value=""/> -->
							<?php //$all_plants = getAllRices();?>

<!-- 							<select id="filter_plant" name="filter_plant">  -->
<!-- 								<option></option> -->
								<?php //foreach ($all_plants as $plant):?>
									<?php //$selected = $plant->PLANT_NAME==$filter_plant ? " selected=\"selected\" " : "";?>
<!--									<option value="<?php //echo $plant->PLANT_NAME?>" <?php //echo $selected?> ><?php //echo $plant->PLANT_NAME?></option> -->
								<?php //endforeach?>
<!-- 							</select> -->
<!-- 						</div> -->
				...
				<div id="top" style="width: 1000px; height: 600px;"></div>
...
// 	google.charts.setOnLoadCallback(drawStuffColumn);
	...
	function drawStuffColumn(count,countall,filter_year,filter_province_hidden,filter_range_income_hidden,filter_status) {
	    ...
	    view.setColumns([0, 1,
	  	...
	  	var options = {
		    width: 1000,
		    heigh: 500,
		    title: 'ข้อมูลการปลูกพันธุ์ข้าวชนิดต่างๆ',
		    bar: { groupWidth: "50%" }
	  	};
		  ...
		  btnSave.addEventListener('click', function () {
				...
				pdfMake.createPdf(docDefinition).download('รายงานข้อมูลพันธุ์ข้าว.pdf');
			  }, false);
	...
	$('#filter_plant').each(function( index ) {
		$(this).select2({
			  placeholder: "เลือกชนิดพันธ์ข้าว"
			});
	});
	...
	function call_ajax(filter_year,filter_province_hidden,filter_range_income_hidden)
	{
		...
		$.ajax({
			url:'<?php echo site_url('ReportAnalytic/count_calrose_rice') ?>',
					dataType:'json',
					type:'GET',		
					...
					success:function (result) {
						count = result.count;
						data_json = [['Move','จำนวน(ไร่)ปลูกแล้ว','จำนวน(ไร่)จะปลูก']];
