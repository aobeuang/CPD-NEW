...
class Report2new extends MY_Controller {
...
// 				_export_to_excel($surveys);
	...
	public function index4new()
	{
				...
				&& (canViewReport() || canAdd()))
		{
			...
			echo $this->load->view('reports2/report4new', $output, TRUE);
			echo $this->load->view('auth/page_footer', '', TRUE);
		}
		else {
			redirect('/');
...
// 			include(APPPATH."third_party/mpdf/mpdf.php");
    ...
    public function queryAuthKhet() {


        try {
            $userId = $this->session->userdata('auth_user_id');
            log_message('info', ' *************** queryAuthKhet userId : '.$userId.' *************** ');
...
//                $userIdInt = (int)$userId;
                log_message('debug', 'begin pkg_master_query.query_auth_khet(:p_cur_result, :p_user_id); end;');

                $stmt = oci_parse($this->db->conn_id, "begin pkg_master_query.query_auth_khet(:p_cur_result, :p_user_id); end;");
...
//                log_message('debug', 'oci_fetch_all. : '.json_encode($data));
    ...
    // Report2new/report4new
    public function queryAuthProvince() {


        try {
            $userId = $this->session->userdata('auth_user_id');
            $khetCode = $this->input->get('khetCode');
            log_message('info', ' *************** queryAuthProvince userId : '.$userId.' *************** ');
            ...
            try {
                ...
                log_message('debug', 'begin pkg_master_query.query_auth_khet(:p_cur_result, :p_user_id, :p_khet_code); end;');

                $stmt = oci_parse($this->db->conn_id, "begin pkg_master_query.query_auth_province(:p_cur_result, :p_user_id, :p_khet_code); end;");
                oci_bind_by_name($stmt, ":p_cur_result", $refcur,  -1,OCI_B_CURSOR);
                oci_bind_by_name($stmt, ":p_user_id", $userId, -1, SQLT_CHR);
