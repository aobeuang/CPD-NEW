...
// 				_export_to_excel($surveys);
	...
	public function getTotalResultNomalOrDieOfCoop($export = false,$filter_khet=null,$filter_provinces=null,$filter_district=null,$filter_coop=null,$life_status=null,$start=0,$length=-1)
	{
		...
		$orderBy = " ORDER BY COOP_INFO.PROVINCE_NAME,COOP_INFO.AMPHUR_NAME";
...
// 		$draw = 1;
	...
	public function getlistProvince($khet_id=null ,$retrun_data = false)
	{
		...
		$province_name = getProvinceByName($province_name_user_search['province']);
		
		$org_id = null;
		if("กรุงเทพมหานคร พื้นที่ 1" == $province_name_user_search['province']){
...
// 		print_r($khet_id);
		...
		foreach ($province as $v)
		{
			if($this->session->userdata('auth_role') == "notcentral_normal")
			{
				...
				if(empty($org_id) && intval($province_name->PROVINCE_ID) == intval($v['COL006']))
				{
					$temp_data[] = array('id'=>$v['COL011'],'name'=>$v['COL007']);
				}else{
					continue;
...
// 				$temp_data[]['name'] = $v['COL008'];
	...
	public function getlistKhet()
	{
		...
		}else if("กรุงเทพมหานคร พื้นที่ 2" == $province_name_user_search['province'])
		{
			...
			if($this->session->userdata('auth_role') == "notcentral_normal" || $this->session->userdata('auth_role') == "notcentral_manager")
				$this->db->where('COL006',$province_name->PROVINCE_ID);
